%% Basin (or rectangular box) average of GPCP data (precipitation ONLY)
%GPCP Version 2.2 Combined Precipitation Data Set"
% The current data set provides two final products, the combined
% satellite-gauge (SG) precipitation estimate and the combined
% satellite-gauge precipitation error estimate. The complete data set,
% which includes the input and intermediate data files, contains a suite of
% 27 products providing monthly, global gridded values of precipitation
% totals and supporting information for the period January 1979 ? (delayed)
% present.
...
for i_site = 17:17%%12:15%1:num_sites
%% Start clean
...
elseif i_site == 11
    % 15. Lake Titicaca-Rio Mauri **lat-lon box
    area_txt = 'Lake Titicaca-Rio Mauri'; basin_name = 'LakeTiticaca'; flag_basin_shpfile = 2; 
    lonmin = -76; lonmax = -64; latmin = -22; latmax = -12;
...
elseif i_site == 12
    ...
    lonmin = 20; lonmax = 40; latmin = -14; latmax = 4; 
    lon_region_min = 28; lon_region_max = 33; lat_region_min = -10; lat_region_max = -2; 
...
if (flag_basin_shpfile == 0) % Grid-based dataset (UNH/GRDC basins)
    ...
    if flag_worldmap
        ...
        axesm('MapProjection', map_proj_name, 'MapLonLimit', lonlim, 'MapLatLimit', ...
            latlim, 'Grid', 'off', 'FontSize', 16, ...
            'FontName', 'helvetica', 'FontWeight', 'bold'); shading flat;
        tightmap;
    end
    pcolorm(lat_basin-nanmean(diff(lat_basin))/2, ...
...
%% GPCP Data
dataset_name='atmos'; var_name='prcp'; unit_name='mm/day'; L=0:1:9;L=L*1.5; c_flag=20; title_name = 'Precipitation'; 
gpcpfilename='gpcp_22.nc';

% Create Year and Month Vectors for gpcp.
mon_vect=repmat([1:12]',32,1);
yr_vect=floor((1979:(1/12):2010.98)');


% Pull out longitude and latitude
lon_data=ncread([basefolder gpcpfilename],'lon');
lat_data=ncread([basefolder gpcpfilename],'lat');
lon_data = [lon_data(73:144)-360;lon_data(1:72)];

%% Create Basin Mask
if flag_basin_shpfile == 0 % Gridded basin dataset
    % realign latitude of data
    lat_data = flipud(lat_data);
...
%     figure; 
%     pcolorm(double(lat_data),double(lon_data),basin_mask);
%     title('model land fraction, basin mask'), colorbar, grid off, shading flat;
...
for curryear=gpcp_years;
    ...
    for currmonth=mon_comp
        ...
        gpcp_varold=ncread([basefolder gpcpfilename],var_name,...
            [1 1 1 yrcut],[ 144 72 1 1]);
        
        % Replace missing values with NaNs
        missing_value = ncreadatt([basefolder gpcpfilename],var_name,'_FillValue');
        gpcp_varold(find(gpcp_varold==missing_value))=NaN;
        
        % Realign and scale
        gpcp_varold = rot90(gpcp_varold,3);
        gpcp_var=[fliplr(gpcp_varold(:,1:72)) fliplr(gpcp_varold(:,73:144))];
%         figure; pcolor(lon_data,lat_data,gpcp_var),title(['CMAP ' monthName{i_mon} ' ' title_name]),colorbar, grid off; shading flat;
 
       if flag_basin_shpfile == 0 % gridded basin data

            var_basin=flipud(gpcp_var).*basin_mask;
       ...
       elseif flag_basin_shpfile == 1 % shapefile

            var_basin=gpcp_var.*basin_mask;
            % Find Latitude and Longitudes for region
            [i_lat]=find(isnan(nanmean(var_basin,2))==0); lat_basin_out=lat_data(i_lat);
            [i_lon]=find(isnan(nanmean(var_basin,1))==0); lon_basin_out=lon_data(i_lon);
            ...
            [cosmean,cosgrid,datagrid,new_lon,new_lat] = coswt(var_basin,lat_data,lon_data,...
                min(lon_basin_out),max(lon_basin_out),min(lat_basin_out),max(lat_basin_out));
%             figure; pcolor(new_lon,new_lat,datagrid),title(['gpcp ' monthName{i_mon} ' ' var_name]),colorbar, grid off; shading flat;
%             figure; pcolor(new_lon,new_lat,double(cosgrid)),title(['gpcp ' monthName{i_mon} ' ' var_name]),colorbar, grid off; shading flat;

        elseif flag_basin_shpfile ==2 % LAT-LON BOX
            % Cosine Weighted Average
            [cosmean,cosgrid,datagrid,new_lon,new_lat] = coswt(gpcp_var,lat_data,lon_data,lonmin,lonmax,latmin,latmax);
            %figure; pcolor(new_lon,new_lat,datagrid),title(['gpcp ' monthName{i_mon} ' ' var_name]),colorbar, grid off; shading flat;
            %figure; pcolor(new_lon,new_lat,cosgrid),title(['gpcp ' monthName{i_mon} ' ' var_name]),colorbar, grid off; shading flat;
        end
        
        cosmean = double(cosmean);
        cosgrid = double(cosgrid);
        
        VAR_gpcp(i_year,i_mon)=cosmean*eomday(gpcp_years(i_year),i_mon);
        VAR_gpcp_grid(i_year,i_mon,:,:)=flipud(gpcp_var)*eomday(gpcp_years(i_year),i_mon);%datagrid*eomday(gpcp_years(i_year),i_mon);
...
gpcpdata.('lon_basin')=lon_data;%lon_basin_out;
gpcpdata.('lat_basin')=lat_data;%lat_basin_out;;
gpcpdata.('yrs')=gpcp_years;
gpcpdata.('months')=monthName{mon_comp};
gpcpdata.('basin_mean')=VAR_gpcp; % average across the watershed
gpcpdata.('basin_grid')=VAR_gpcp_grid; % spatial data, limited to the watershed
save([basefolder var_name '.' basin_name '.gpcp.mat'],'gpcpdata');
 
%% Test
worldmap('World');
%axesm('MapProjection', map_proj_name, 'MapLatLimit', latlim, 'MapLonLimit', lonlim);
pcolorm(double(lat_data),double(lon_data),VAR_gpcp_grid);
title('model land fraction, basin mask'), colorbar, grid off, shading flat;
%% Plot
figure
hold on
plot(mon_comp,mean(VAR_gpcp),'Color','b','LineStyle','-','LineWidth',3,'Marker','none');
...
legend(['GPCP V2.2'],'Location','NorthEast');
box on
hold off
